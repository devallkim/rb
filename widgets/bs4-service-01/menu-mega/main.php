<?php
if (!function_exists('getMenuWidgetMega'))
{
	function getMenuWidgetMega($site,$table,$is_child,$parent,$depth,$id,$w,$_C)
	{
		global $_CA;
		...
		{
?>
<ul class="nav text-center nav-justified">
	<?php
	...
	?>
	<li class="nav-item<?php if($_isActive):?> active<?php endif?>">
		<a href="<?php echo RW('c='.$_newTree.'/'.$C1['id'])?>" class="nav-link"<?php if($C1['target']):?> target="<?php echo $C1['target']?>"<?php endif?>>
			<?php echo $C['name']?>
		</a>
		<ul class="nav flex-column my-3">
			<?php while($C1 = db_fetch_array($CD1)):?>
			<li class="nav-item"><a class="nav-link<?php if(in_array($C1['id'],$_CA)):?> active<?php endif?>" href="<?php echo RW('c='.$_newTree.'/'.$C1['id'])?>" <?php if($C1['target']):?> target="<?php echo $C1['target']?>"<?php endif?>><?php echo $C1['name']?></a></li>
			<?php endwhile?>
		</ul>
	</li>
	<?php endwhile?>
...
getMenuWidgetMega($s,$table['s_menu'],0,$wdgvar['smenu'],$wdgvar['depth'],$wdgvar['sid'],$wdgvar,array());
?>
