		...
		<div class="form-group form-row">
			...
			<div class="col-md-11">
				...
				<div class="btn-group btn-group-lg" data-toggle="buttons">
					<a href="#usemobile-00" class="btn btn-light<?php if(!$R['usemobile']):?> active<?php endif?>" style="height:46px;font-size:18px;" data-toggle="tab">
						<input type="radio" name="usemobile" value="0"<?php if(!$R['usemobile']):?> checked="checked"<?php endif?>> 사이트별 모바일모드 적용
					</a>
					<a href="#usemobile-02" class="btn btn-light<?php if($R['usemobile']==2):?> active<?php endif?>" style="height:46px;font-size:18px;" data-toggle="tab">
						<input type="radio" name="usemobile" value="2"<?php if($R['usemobile']==2):?> checked="checked"<?php endif?>> 특정 도메인 으로 이동
					</a>
					<a href="#usemobile-01" class="btn btn-light<?php if($R['usemobile']==1):?> active<?php endif?>" style="height:46px;font-size:18px;" data-toggle="tab">
						<input type="radio" name="usemobile" value="1"<?php if($R['usemobile']==1):?> checked="checked"<?php endif?>> 특정 사이트로 연결
					</a>
				</div>
				<div class="tab-content">
					<div class="tab-pane well fade<?php if(!$R['usemobile']):?> show active<?php endif?>" id="usemobile-00">
						<p class="form-control-static">
							모바일 기기로 접속 시 사이트별로 모바일 모드를 적용할 수 있습니다.
						</p>
					</div>
					<div class="tab-pane well fade<?php if($R['usemobile']==2):?> show active<?php endif?>" id="usemobile-02">
