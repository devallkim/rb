      ...
      <div id="pagegroup">
				...
				<div class="subnav clearfix my-3">
					<div class="subnav-links">
						<a class="subnav-item selected" onclick="catFlag('');">가입그룹 <small>694</small></a>
						<a class="subnav-item" onclick="catFlag('1');">관리그룹 <small>508</small></a>
						<a class="subnav-item" onclick="catFlag('2');">개설그룹 <small>186</small></a>
						<a class="subnav-item" onclick="catFlag('2');">추천그룹 <small>186</small></a>
					</div>
				</div>
      	...
      	$orderby= 'asc';
      	$recnum	= 15;
      	if ($where && $keyword) $bbsque_group .= getSearchSql($where,$keyword,$ikeyword,'or');
      	$RCD = getDbArray($table['forummembers'],$bbsque_group,'*',$sort,$orderby,$recnum,$p);
      	$NUM = getDbRows($table['forummembers'],$bbsque_group);
      	...
      	<tr>
      	<td class="l1"><a href="<?php echo $g['s']?>/group/<?php echo $R['id']?>"><img src="<?php if($R['imgsymbol']):?><?php echo $g['s']?>/modules/forum/var/files/<?php echo $R['imgsymbol']?><?php else:?><?php echo $g['img_layout']?>/nopic_group_50.gif<?php endif?>" width="50" height="50" alt="" /></a></td>
      	<td class="l2">
      		<a href="<?php echo $g['s']?>/group/<?php echo $R['id']?>" class="sbj"><?php echo $R['name']?></a>
      	...
      	<td class="l3">
      		<?php if($R['ismember']):?>
      		<img src="<?php echo $g['img_layout']?>/btn_group_joing.gif" alt="가입중" onclick="alert('이미 가입하셨습니다.     ');" />
      		<?php else:?>
      		<?php if($my['uid']):?>
      		<a href="<?php echo $g['s']?>/?r=<?php echo $r?>&amp;m=<?php echo $m?>&amp;bid=<?php echo $R['id']?>&amp;mod=join"><img src="<?php echo $g['img_layout']?>/btn_group_join.gif" alt="가입하기" /></a>
      		<?php else:?>
      		<img src="<?php echo $g['img_layout']?>/btn_group_join1.gif" alt="가입하기" onclick="alert('로그인 후 가입하실 수 있습니다.     ');" />
      		<?php endif?>
      		<?php endif?>
      	</td>
      	</tr>
      	...
      	<?php if(!$NUM):?>
      	<div class="none">해당되는 그룹이 없습니다.</div>
      	<?php endif?>
      	...
      	<div class="searchform">
      		...
      		<select name="where">
      		<option value="name"<?php if($where=='name'):?> selected="selected"<?php endif?>>그룹주제</option>
      		<option value="id"<?php if($where=='id'):?> selected="selected"<?php endif?>>그룹코드</option>
      		</select>
