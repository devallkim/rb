...
if ($act=='send_code') {
	...
	$content = str_replace('{EMAIL}',$my['email'],$content);
	$content.= '<p>'.$R['email'].' 본인확인 인증번호  : '.$verify_code.'</p>';
	$content.= implode('',file($g['dir_module'].'doc/_sign.txt'));

	$result = getSendMail($R['email'].'|'.$my['name'], $d['member']['join_email'].'|'.$_HS['name'], '['.$R['email'].'] 본인확인용 인증번호가 발급되었습니다.', $content, 'HTML');
...
if ($act=='save_primary') {
	...
	getDbUpdate($table['s_mbremail'],'prim=0','mbruid='.$my['uid']); // 기본 이메일 초기화
	getDbUpdate($table['s_mbremail'],'prim=1','uid='.$R['uid'].' and mbruid='.$my['uid']); // 기본 이메일 지정
...
if ($act=='confirm_code') {
	...
	if (!$my['email']) {
		getDbUpdate($table['s_mbremail'],'prim=1','uid='.$uid.' and mbruid='.$my['uid']); // 기본 휴대폰 지정
