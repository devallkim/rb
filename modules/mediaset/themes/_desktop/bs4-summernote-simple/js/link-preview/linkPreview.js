...
(function ($) {
    $.fn.linkPreview = function (options) {
        ...
        selector = selector.substr(1); // # or . 제거 (class or id 이름만 취한다.)
        
        // theme  를 배열로 넘긴 후 opts 를 통해서 치환하면 적용된다. 
        var loader='<div style="height:50%;margin-top:40%;" id="modal-loader-default"><div class="spinner-wrap"><div class="spinner"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div></div></div>';
        ...
        $(this).append('<div class="link-preview-wrapper rb-imported"><textarea id="text_' + selector + '" placeholder="' + opts.placeholder + '" rows="5" name="link_content" data-role="focus"/></textarea> <div id="preview_' + selector + '" class="media preview-box" data-role="link-content"> <div id="previewImages_' + selector + '" class="media-left"> <div id="previewImage_' + selector + '" class="previewImage">'+loader+'</div><input type="hidden" id="photoNumber_' + selector + '" class="photoNumber" value="0"/> </div><div id="previewContent_' + selector + '" class="media-body pContent-wrap"> <div id="closePreview_' + selector + '" title="Remove" class="closePreview" ><i class="fa fa-times"></i></div><div id="previewTitle_' + selector + '" class="previewTitle"></div><div id="previewUrl_' + selector + '" class="previewUrl"></div><p id="previewDescription_' + selector + '" class="rb-description"></p><div id="hiddenDescription_' + selector + '" class="hiddenDescription"></div></div></div></div>');
        ...
        // 마크다운으로 변환 함수 
        var getMarkdown=function(html){
            var md_option=null;
            var md_content=toMarkdown(html,md_option); // 마크다운으로 변환함수 호출 : list.php 에 to-markdown 플로그인 
            return md_content;
        }

        // 링크내용 추출 함수 
        var getLinkContent=function(title,url,img,description){
             var content='<div class="media"><div class="media-left"><div class="previewImage"><img src="'+img+'"></div></div><div class="media-body"><div class="previewTitle">'+title+'</div><div class="previewUrl">'+url+'</div><div class="rb-description">'+description+'</div></div>';
        ...
        var crawlText = function () {
            ...
            if (trim(text) !== "") {
                ...
                if (block === false && urlRegex.test(text)) {
                    ...
                    $('#previewButtons_' + selector).hide();
                    // 별도 로더 적용 
                    //$('#previewLoading_' + selector).html("<img src='"+loader+"' />");
                    ...
                    $.post(rooturl+'/?m='+module+'&a=start_link_preview', {
                        text: text,
                        theme : theme,
                        ...
                        // 링크 content 추출 
                        var insert_content=getLinkContent(answer.title,answer.url,answer.images,answer.description);      
                        var md_content=getMarkdown(insert_content);                 
                        $('[data-role="btn-copy-linkContent"]').attr('data-clipboard-text',md_content); //  복사 버튼의 data-clipboard-text 값에 입력 
                       
                        // 다른 로더 적용
                        //$('#previewLoading_' + selector).html("");
                        $('.rb-status .rb-spinner').hide(200); 
                        $('#preview_' + selector).show();
                        $('#previewTitle_' + selector).html("<span id='previewSpanTitle_" + selector + "' class='previewSpanTitle' >" + answer.title + "</span><div contenteditable='true'  id='previewInputTitle_" + selector + "' class='previewInputTitle inputPreview' style='display: none;'>" + answer.title + "</div>");
                        // $('#text_' + selector).css({
                        ...
                        // textarea 문제로 contenteditable 로 변경 
                        ...
                        $('#previewDescription_' + selector).html("<span id='previewSpanDescription_" + selector + "' class='previewSpanDescription' >" + answer.description + "</span><div contenteditable='true' id='previewInputDescription_" + selector + "' class='previewInputDescription' style='display: none;' class='inputPreview' >" + answer.description + "</div>");
                        ...
                        $('#previewSpanTitle_' + selector).unbind('click').click(function (e) {
                            ...
                            if (blockTitle === false) {
                                ...
                                $('#previewInputTitle_' + selector).val($('#previewInputTitle_' + selector).text());
                                $('#previewInputTitle_' + selector).focus().select();
                            }
                        });
                        $('#previewInputTitle_' + selector).blur(function () {
                            blockTitle = false;
                            $('#previewSpanTitle_' + selector).html($('#previewInputTitle_' + selector).text());
                        ...
                        $('#previewSpanDescription_' + selector).unbind('click').click(function (e) {
                            ...
                            if (blockDescription === false) {
                                ...
                                $('#previewInputDescription_' + selector).css("width","100%");                                
                                //$('#previewInputDescription_' + selector).height(300);
                                $('#previewInputDescription_' + selector).val($('#previewInputDescription_' + selector).text());
                                $('#previewInputDescription_' + selector).focus().select();
                            }
                        });
                        $('#previewInputDescription_' + selector).blur(function () {
                            blockDescription = false;
                            $('#previewSpanDescription_' + selector).html($('#previewInputDescription_' + selector).text());
            ...
            if (((trim(text) !== "") || (trim(text) === "" && trim(hrefUrl) !== "")) && (allowPosting === true && isCrawling === false)) {
                $.get('php/highlightUrls.php', {
                    ...
                    $.post('php/save.php', {
                        ...
                        content = '<div class="previewPosted" id="contentWrap' + id + '" >' + '<div class="previewTextPosted">' + urls.urls + '</div>' + videoIframe + '<div class="previewImagesPosted">' + '<div class="previewImagePosted">' + leftSideContent + '</div>' + '</div>' + '<div class="previewContentPosted">' + '<div class="previewTitlePosted" id="' + pTP + '" style="width: ' + contentWidth + '%" ><a href="' + hrefUrl + '" target="_blank">' + title + '</a></div>' + '<div class="previewUrlPosted">' + fancyUrl + '</div>' + '<div class="previewDescriptionPosted" id="' + pDP + '" style="width: ' + contentWidth + '%" >' + urls.description + '</div>' + '</div><div style="clear: both"><span style="color: red; cursor: pointer; float: right" id="delete_action_' + id + '" ref="' + id + '">delete</span></div></div>';
