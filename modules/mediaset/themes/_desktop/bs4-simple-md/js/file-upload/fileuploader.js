...
(function ($) {
	...
	$.fn.RbUploadFile = function (options) {
		...
		$(uploadLabel).addClass(s.uploadButtonClass);
            ...
            // <input file > id 값 규정
            var fileUploadId;
            var event_handler=s.event_handler;
            var event_type=$(event_handler).data('type');
            if(event_type=='photo') fileUploadId='attach-input-photo';
						if(event_type=='video') fileUploadId='attach-input-video';
						if(event_type=='audio') fileUploadId='attach-input-audio';
            else fileUploadId='attach-input-file';

	       // 업로드 버튼 클릭 이벤트 바인딩
	      $(s.event_handler).click(function(e){
	              e.preventDefault();
	              $('#'+fileUploadId).click();
	      });
      ...
      if(event_type=='photo') fileUploadId='attach-input-photo';
		...
		function ajaxFormSubmit(form, s, pd, fileArray, obj, file) {
			...
			var event_handler=s.event_handler;
			var event_type=$(event_handler).data('type');
			var btn_upload=$('[data-role="attach-handler-'+event_type+'"]');
			var btn_loading_text=$(btn_upload).data('loading-text'); // 업로드 중....
			var btn_origin_text=$(btn_upload).data('label'); // 업로드
			var currentXHR = null;
			var options = {
				cache: false,
				contentType: false,
				...
				beforeSend: function (xhr, o) {
					...
					// 업로드 로딩 표시 하기 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
          $(btn_upload).find('span').text(btn_loading_text);
				...
				success: function (data, message, xhr) {
					...
					// 업로드 완료 표시  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
          $(btn_upload).find('span').text(btn_origin_text);
