<div id="attach-form-audio"><!-- 파일폼 출력 --></div>
<div data-role="rb-attach-wrapper-audio">
    <div class="rb-attach">
          <ul class="list-group list-group-flush rb-attach-audio" data-role="attach-preview-audio"><!-- 오디오 리스트  -->
    ...
    <!-- 첨부 오디오 메타정보 수정 -->
    <div class="modal fade rb-modal-attach-meta" id="modal-attach-audio-meta" tabindex="-1" role="dialog" aria-labelledby="">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title" id=""><i class="fa fa-volume-up"></i> 첨부 오디오 정보수정</h4>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-4"><h1 class="text-center"><i class="fa fa-volume-up fa-4x"></i></h1></div>
                        <div class="col-md-8">

                                <audio controls="" preload="auto" src="" style="width: 100%"></audio>
                                <hr>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-light" data-dismiss="modal" data-attach-act="cancel" data-target="#modal-attach-audio-meta" data-role="eventHandler" data-id="">취소하기</button>
                    <button type="button" class="btn btn-primary" data-attach-act="save-file" data-target="#modal-attach-audio-meta" data-role="eventHandler" data-id="" data-type="">저장하기</button>
                </div>
            </div>
        </div>
    </div>
</div>
<script src="<?php echo $g['url_attach_theme']?>/main-audio.js"></script>
<script>
// fileuploader.js 중복로드 방지
//if($.fn.RbUploadFile == undefined) $.getScript("<?php echo $g['url_attach_theme']?>/js/file-upload/fileuploader.js");

$('#modal-attach-audio-meta').appendTo('body');

var inputFileId='attach-input-audio'; // 실제 작옹하는 input 엘리먼트 id 값을 옵션으로 지정을 해준다. (커스텀 버튼으로 click 이벤트 바인딩)
...
$(document).ready(function()
{
       ...
       var file_upload_settings = {
           //allowedTypes:"jpg,png,gif,hwp,doc,pdf,ppt,pptx,xls,xlsx,zip",// 업로드 가능한 파일 확장자. 여기에 명시하지 않으면 파일 확장자 필터링하지 않음.
            ...
            event_handler : '[data-role="attach-handler-audio"]', // 업로드 버튼  엘리먼트
            formData: {"saveDir":attach_file_saveDir,"theme":attach_module_theme} // 추가 데이타 세팅
       }
       $("#attach-form-audio").RbUploadFile(file_upload_settings); // 아작스 폼+input=file 엘리먼트 세팅
       ...
       // main.js 기본값 세팅 :
       $('[data-role="rb-attach-wrapper-audio"]').RbAttachFile(attach_file_settings);


       $('#modal-attach-audio-meta').on('shown.bs.modal', function (event) {
         ...
         var src = button.attr('data-src'); // data-로 하면 변경된 값 적용 안됨
        //  var caption = button.attr('data-caption'); // data- 로 하면 변경된 값 적용 안됨
         ...
         modal.find('audio').attr('src',src); // mp3 경로 추가

       })

       $('#modal-attach-audio-meta').on('hidden.bs.modal', function (event) {
         ...
         $(this).find('audio').attr('src',''); // 입력된 캡션명 초기화
       })
