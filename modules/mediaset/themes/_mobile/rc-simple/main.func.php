...
function getAttachAudio($R,$mod,$featured_audio_uid)
{
	  ...
	  $html.='
		<li class="table-view-cell media" data-id="'.$R['uid'].'" data-role="attach-item">
				...
				if($mod=='upload'){
				$html.='
				<button type="button" class="btn btn-secondary btn-sm dropdown-toggle hidden" data-toggle="dropdown">
						<span class="caret"></span>
						<span class="sr-only">Toggle Dropdown</span>
				</button>
				<ul class="dropdown-menu dropdown-menu-right hidden" role="menu">';
									$html.='
		...
		$html.='<h6 class="media-heading"><span data-role="attachList-list-name-'.$R['uid'].'" >'.$R['name'].'</span>';
		$html.='<span class="badge badge-secondary'.($R['uid']==$featured_audio_uid?'':' hidden-xs-up').'" data-role="attachList-label-featured" data-id="'.$R['uid'].'">대표</span> ';
	  $html.='<span class="badge badge-secondary'.(!$R['hidden']?' hidden-xs-up':'').'" data-role="attachList-label-hidden-'.$R['uid'].'">숨김</span>';
		$html.='
		<span class="badge badge-secondary">'.getSizeFormat($R['size'],2).'</span></h6><p><audio controls class=""><source src="'.$R['url'].$R['folder'].'/'.$R['tmpname'].'" type="audio/'.$R['ext'].'"></audio></p></div></li>';
		return $html;
}
...
function getAttachVideo($R,$mod,$featured_video_uid)
{
	  ...
	  $html.='
		...
		$html.='<span class="badge badge-secondary'.($R['uid']==$featured_video_uid?'':' hidden-xs-up').'" data-role="attachList-label-featured" data-id="'.$R['uid'].'">대표</span> ';
	  $html.='<span class="badge badge-secondary'.(!$R['hidden']?' hidden-xs-up':'').'" data-role="attachList-label-hidden-'.$R['uid'].'">숨김</span></h5	>';
