$(function () {
  ...
  $('[data-toggle="getMemberLayer"]').popover({
    ...
    content: function () {
      ...
      if (!theme) theme = '_desktop/bs4-simple'
      $.post(rooturl+'/?r='+raccount+'&m=member&a=get_profileData',{
         ...
         $('#item-'+uid).html(profile);
       });
      return '<div id="item-'+uid+'" class="p-1">불러오는 중...</div>';
    }
  })
...
var doBbsList = function(){
  ...
  $('#modal-bbs-view').on('show.bs.modal', function(event) {
    ...
    $.post(rooturl+'/?r='+raccount+'&m=bbs&a=get_postData',{
       ...
       var theme = '_desktop/bs4-default';
   ...
   //좋아요,싫어요
   $('#modal-bbs-view').on('click','[data-toggle="opinion"]',function(){
   ...
   //게시물보기 모달이 닫혔을 때
   $('#modal-bbs-view').on('hidden.bs.modal', function() {
      var modal = $(this);
