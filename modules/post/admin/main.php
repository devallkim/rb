...
if ($blogtype) $blogque .= ' and blogtype='.$blogtype;
if ($where && $keyw)
{
	if (strstr('[id]',$where)) $blogque .= " and ".$where."='".$keyw."'";
	else $blogque .= getSearchSql($where,$keyw,$ikeyword,'or');
}
...
<div class="container-fluid">
	<div class="row">
				   ...
				   <div id="search-more" class="collapse<?php if($_SESSION['sh_mediaset']):?> in<?php endif?>">
				       <div class="form-group">
							 ...
							 <div class="col-sm-10">
								 <div class="row">
									<div class="col-sm-3">
										<select name="blogtype" onchange="this.form.submit();" class="form-control input-sm">
											<option value=""<?php if($blogtype==''):?> selected="selected"<?php endif?>>전체(<?php echo getDbRows($table[$module.'list'],'')?>)</option>
											<option value="1"<?php if($blogtype==1):?> selected="selected"<?php endif?>>개인블로그(<?php echo getDbRows($table[$module.'list'],'blogtype=1')?>)</option>
											<option value="2"<?php if($blogtype==2):?> selected="selected"<?php endif?>>팀블로그(<?php echo getDbRows($table[$module.'list'],'blogtype=2')?>)</option>
										</select>
									</div>
								   <div class="col-sm-3">
		...
		<div class="col-sm-8 col-md-8 ml-sm-auto col-xl-9 pt-3">
			...
			<h4>포스트셋  리스트
				<a href="#" data-toggle="modal" data-target="#modal_window" onmousedown="blogUidSet('<?php echo $R['uid']?>','makeblog');" class="pull-right btn btn-link rb-modal-blog">
					<i class="fa fa-plus"></i> 새 포스트셋  만들기
				</a>
			</h4>
			...
			<form name="listForm" action="<?php echo $g['s']?>/" method="post">
					...
					<div class="table-responsive">
						<table class="table table-striped">
							...
							<tr>
								<td><input type="checkbox" name="blog_members[]" value="<?php echo $R['uid']?>" class="rb-list-td" onclick="checkboxCheck();"/></td>
							   ...
							   <td><a href="<?php echo $g['s']?>/?r=<?php echo $r?>&amp;m=<?php echo $module?>&amp;pset=<?php echo $R['id']?>" target="_blank"><?php echo $R['id']?></a></td>
								...
								<td>
									<a href="<?php echo $g['s']?>/?r=<?php echo $r?>&amp;m=<?php echo $module?>&amp;a=_admin/deleteblog&amp;uid=<?php echo $R['uid']?>" onclick="return hrefCheck(this,true,'삭제하시면 모든 포스트가 지워지며 복구할 수 없습니다.\n정말로 삭제하시겠습니까?');" class="del">삭제</a>
									<a href="#" data-toggle="modal" data-target="#modal_window" class="rb-modal-blog" onmousedown="blogUidSet('<?php echo $R['uid']?>','makeblog');">설정</a>
					...
					<div class="rb-footer clearfix">
						...
						<div>
							<button type="button" onclick="chkFlag('blog_members[]');checkboxCheck();" class="btn btn-default btn-sm">선택/해제</button>
...
$('.rb-modal-blog').on('click',function() {
	modalSetting('modal_window','<?php echo getModalLink('&amp;m=admin&amp;module=post&amp;front=modal.blog&amp;uid=')?>'+_blogUid+'&amp;_front='+_blogFront);
});
