...
$BLOGS = getDbArray($table[$m.'list'],'mbruid='.$my['uid'],'*','gid','asc',0,$p);
$ISCAT = getDbRows($table[$m.'category'],'blog='.$uid);
...
<div id="mainbox">
	...
	<div class="category">
		<div class="title">
			...
			<select onchange="goHref('<?php echo $g['adm_href']?>&uid='+this.value);">
		...
		var ulink = "<?php echo $g['adm_href']?>&amp;uid=<?php echo $uid?>&amp;cat=";
		//]]>
		</script>
		<script type="text/javascript" src="<?php echo $g['s']?>/_core/js/tree.js"></script>
		<script type="text/javascript">
		//<![CDATA[
		var TREE_ITEMS = [['', null, <?php getMenuShowBlog($uid,$table[$m.'category'],0,0,0,$cat,$CXA,0)?>]];
		...
		<input type="hidden" name="a" value="admin/user_modifymenugid" />
		...
		<ul id="menuorder">
		<?php $_MENUS=getDbSelect($table[$m.'category'],'blog='.$uid.' and parent='.intval($CINFO['uid']).' and depth='.($CINFO['depth']+1).' order by gid asc','*')?>
	...
	<div class="catinfo">
		...
		<input type="hidden" name="r" value="<?php echo $r?>" />
		<input type="hidden" name="m" value="<?php echo $m?>" />
		<input type="hidden" name="a" value="admin/user_regiscategory" />
		...
		<div class="title">
			...
			<div class="xright">
				...
				<a href="<?php echo $g['adm_href']?>&amp;uid=<?php echo $uid?>&amp;type=makecategory">최상위 카테고리 등록</a>
		...
		<table>
			...
			<tr>
				...
				<a href="<?php echo $g['adm_href']?>&amp;uid=<?php echo $uid?>&amp;cat=<?php echo $ctarr[$i]['uid']?>"><?php echo $ctarr[$i]['name']?></a>
			...
			<tr>
				...
				<td class="td2">
					...
					<?php if($is_fcategory):?>
					<span class="btn01"><a href="<?php echo $g['s']?>/?r=<?php echo $r?>&amp;m=<?php echo $m?>&amp;a=admin/user_deletecategory&amp;uid=<?php echo $uid?>&amp;cat=<?php echo $cat?>&amp;parent=<?php echo $delparent?>" target="_action_frame_<?php echo $m?>" onclick="return confirm('정말로 삭제하시겠습니까?     ')">카테고리 삭제</a></span>
					<span class="btn01"><a href="<?php echo $g['adm_href']?>&amp;uid=<?php echo $uid?>&amp;cat=<?php echo $cat?>&amp;vtype=sub">서브 카테고리 등록</a></span>
					<?php endif?>
				</td>
			</tr>
