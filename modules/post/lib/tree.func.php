...
function getTreeCategory($conf,$code,$depth,$parent,$tmpcode)
{
	...
	$tree = '<div class="rb-tree"><ul id="'.$id.'" class="mb-0">';
	$CD=getDbSelect($conf['table'],($conf['set']?'blog='.$conf['set'].' and ':'').'depth='.($depth+1).' and parent='.$parent.($conf['dispHidden']?' and hidden=0':'').($conf['mobile']?' and mobile=1':'').' order by gid asc','*');
	$_i = 0;
	while($C=db_fetch_array($CD))
	{
...
function getTreeCategoryForWrite($conf,$code,$depth,$parent,$tmpcode)
{
	...
	$CD=getDbSelect($table[$m.'category'],($conf['set']?'blog='.$conf['set'].' and ':'').'depth='.($depth+1).' and parent='.$parent.($conf['dispHidden']?' and hidden=0':'').($conf['mobile']?' and mobile=1':'').' order by gid asc','*');
	$_i = 0;
	while($C=db_fetch_array($CD))
	{
		$rcode= $tmpcode?$tmpcode.'/'.$C[$ctype]:$C[$ctype];
		...
		$is_selected=getDbRows($table[$m.'catidx'],'blog='.$conf['set'].' and post='.$conf['post'].' and category='.$C['uid']);
...
function postLabel($mod,$step,$isreserve)
{
	...
	$step_label=array('secondary','success','danger','secondary');
    $html='';
    ...
    // 예약발행 체크
    if($isreserve) $html.=' <span class="badge badge-info">예약</span>';
...
function getTimeagoDate($date,$mod)
{
	if($date){
	   $html='<time class="timeago" data-toggle="tooltip" datetime="'.getDateFormat($date,'c').'" data-tooltip="tooltip" title="'.getDateFormat($date,'Y.m.d H:i').'"></time>';
		...
		if($mod=='d_published') $html ='<span class="text-gray">발행 전</span>';
		else $html='';
	}
	return $html;
}
...
function getPostCatName($set,$post)
{
    global $table;

    $m='post';
    $catque='blog='.$set.' and post='.$post;
    $CAX=array();
...
// 포스트의 모든 카테고리 출력
function getAllPostCat($project,$post)
{
    global $table;
...
// 포스트의 카테고리 링크값  출력
function getPostCatLink($set,$post)
{
   global $g,$table;
...
function getPostData($R,$data)
{
   ...
   if($d['blog']['rewrite']) $result['link']=$g['s'].($r?'/'.$r:'').'/post/'.$B['id'].'/';
...
//분류코드->SQL
function getBlogCategoryCodeToSql($table,$cat)
{
	$R=getUidData($table,$cat);
	...
	if ($R['isson'])
	{
		...
		while($C=db_fetch_array($RDATA)) $sql .= getBlogCategoryCodeToSqlX($table,$C['uid'],$C['isson']);
	}
	return substr($sql,0,strlen($sql)-4);
}
//분류코드->SQL
function getBlogCategoryCodeToSqlX($table,$cat,$isson)
{
	$sql = 'category='.$cat.' or ';
	if ($isson)
	{
