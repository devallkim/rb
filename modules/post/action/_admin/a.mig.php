...
for($i = 1; $i < $RSS['count']; $i++)
{
	...
	$QVAL1 = "'$set','$gid','0','0','0','0','$mbruid','$tag','$subject','$review','$content',";
	...
	if(!getDbRows($table[$m.'day'],"date='".$d_today."' and set=".$B['uid'])) getDbInsert($table[$m.'day'],'date,set,num',"'".$d_today."','".$B['uid']."','1'");
...
if ($viewresult)
{
	...
	echo "window.open('".$g['s'].'/?r='.$r.'&m='.$m.'&set='.$B['id']."');";
