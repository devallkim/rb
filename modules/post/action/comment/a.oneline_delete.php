...
getDbUpdate($table[$m.'members'],'num_o=num_o-1','set='.$C['set'].' and mbruid='.$R['mbruid']);
getDbUpdate($table[$m.'list'],'num_o=num_o-1','uid='.$C['set']);

if ($d['set']['c_give_opoint']&&$R['mbruid'])
{
	getDbInsert($table['s_point'],'my_mbruid,by_mbruid,price,content,d_regis',"'".$R['mbruid']."','0','-".$d['set']['c_give_opoint']."','한줄의견삭제(".getStrCut(str_replace('&amp;',' ',strip_tags($R['content'])),15,'').")환원','".$date['totime']."'");
